...
public class BusinessForSaveAlarm0 implements Runnable{
	...
	public BusinessForSaveAlarm0(TimlyPojo tp, CachePojo<String, UnitPojo, AreaPojo, DevicePojo, TagPojo, RulePojo, TimlyPojo, VehiclePojo> cache, int unitIndex){
		this.tp = tp;
		this.cache = cache;
	...
	public void run() {
		...
		try{
			...
			DevicePojo dp = cache.getDeviceCache().get(curDev);
			...
			sb.append(tagId);sb.append("', ");sb.append(1);sb.append(", '', '");sb.append(vp.getOwnerName());
